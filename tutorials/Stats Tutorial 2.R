...
class(Trump) #What class is Trump? Supply the object name as an argument
typeof(Trump) #What is Trump a *type of*? Supply the object name as an argument
...
str(Trump) #A more elaborate summary of the *structure* of an object can be obtained with
...
Trump_FOX <- Trump[Trump$survey_house == "FOX",] #subset using square brackets
Trump_Pew <- subset(Trump, survey_house == "Pew")
Trump_Pew <- Trump[Trump$survey_house == "Pew",] #subet using square brackets
...
object1 <- aggregate(Trump$Approve, by = list(Trump$mode), FUN = mean)
